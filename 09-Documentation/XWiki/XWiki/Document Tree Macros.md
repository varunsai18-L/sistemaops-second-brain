---
id: xwiki-xwiki:XWiki.DocumentTreeMacros
type: XWiki Page
space: "XWiki"
author: "XWiki.superadmin"
version: "1.1"
last_modified: 1781905784000
sync_date: 2026-07-21 11:00:57
tags:
  - xwiki/documentation
  - space/xwiki
---
# Document Tree Macros

- **Space:** XWiki
- **Author:** XWiki.superadmin
- **Last Modified:** 1781905784000
- **Source:** [Document Tree Macros](https://wiki.systemaops.in/bin/view/XWiki/xwiki:XWiki.DocumentTreeMacros)
...
#macro (updateDocTreeConfigFromRequest)
  ...
  ## FIXME: The 'orderBy' property of the tree API is shared by all tree node types, which means we can't indicate a
  ## different sort field per tree node type (e.g. sort wiki nodes by name and document nodes by last modification
  ## date). At the same time, this property is currently taken into account only for sorting document tree nodes, so for
  ## now we set its value to the specified document sort. In the future we may want to convert this into a map, where
  ## the key is the node type.
  #set ($docTreeConfig.orderBy = $docTreeConfig.sortDocumentsBy)
  ## Sort the child documents by (raw) title when the node label is the document title and there's no sort specified.
  #if ($docTreeConfig.showDocumentTitle && "$!docTreeConfig.orderBy" == '')
    #set ($docTreeConfig.orderBy = 'title:asc')
  #end
...
#macro (maybeAddFarmNode $nodeReference $siblings)
  ...
  #set ($isOpened = $docTreeConfig.expandToLevel > 0)
...
#macro (addSpaceNode $spaceReference $siblings)
  ...
  #set ($canViewSpace = $services.security.authorization.hasAccess('view', $spaceReference))
    ...
    #set ($hasChildren = $tree.getChildCount($spaceNodeId) > 0)
  #end
  #set ($isOpened = false)
    ...
    #end
    #set ($rootDistance = $tree.getPath($spaceNodeId).size())
...
#macro (addDocumentNode $documentReference $siblings)
  ...
  #set ($docNodeId = "document:$documentId")
